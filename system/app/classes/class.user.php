													...
													if ($stmt->RowCount() < 4) {
														if (self::refUser($_POST['referrer']) || empty($_POST['referrer'])) {
															...
															if ($config['hotelEmu'] == 'arcturus') {
																...
																$addNewUser->bindParam(':avatar', $config['registerLook']);
																...
																$addNewUser = $dbh->prepare("
																			INSERT INTO
																			users
																			(username, password, rank, auth_ticket, motto, account_created, last_online, mail, look, ip_last, ip_register, credits, activity_points, vip_points)
																			VALUES
																			(
																			:username, 
																			:password, 
